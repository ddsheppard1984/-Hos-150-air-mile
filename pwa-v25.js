/* HOS 150 Driver Companion V33 — stable PWA shell + GPS weather fix */
(()=>{'use strict';
const APP='HOS 150 Driver Companion V33';
function brand(){document.title=APP;const d=document.querySelector('meta[name="description"]');if(d)d.content='HOS, truck navigation, GPS, trip planning, weather, driver tools, customer address book and 150-air-mile companion for professional drivers.';document.querySelectorAll('meta[name="app-version"],meta[name="build-id"]').forEach(x=>x.setAttribute('content',x.name==='app-version'?'33.0':'2026-09-10-v33.0'));let m=document.querySelector('link[rel="manifest"]');if(m)m.href='manifest.webmanifest?v=33.0'}
...
function sw(){if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=33.0').catch(()=>{})}
function boot(){brand();theme();visual();installButton();sw();load('customer-routing-v31.js?v=33.0','customer-routing-v31');load('gps-weather-fix-v33.js?v=33.0','gps-weather-fix-v33')}
