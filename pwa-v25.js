/* HOS 150 Driver Companion V30 — installable app shell */
(()=>{'use strict';
const APP='HOS 150 Driver Companion V30';
function brand(){
  document.title=APP;
  const d=document.querySelector('meta[name="description"]');
  if(d)d.content='HOS, truck navigation, GPS, trip planning, weather, driver tools, customer address book and 150-air-mile companion for professional drivers.';
  document.querySelectorAll('meta[name="app-version"],meta[name="build-id"]').forEach(x=>x.setAttribute('content',x.name==='app-version'?'30.0':'2026-09-10-v30.0'));
  ...
  let m=document.querySelector('link[rel="manifest"]');if(m)m.href='manifest.webmanifest?v=30.0';
...
function sw(){if('serviceWorker' in navigator)navigator.serviceWorker.register('./sw.js?v=30.0').catch(()=>{});}
function boot(){brand();sw();loadCustomerEnhancements()}
